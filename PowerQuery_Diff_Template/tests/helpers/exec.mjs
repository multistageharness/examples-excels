...
/** automated/ — where the reference project and the vendored SDK live. */
const AUTOMATED_DIR = path.resolve(HERE, '../../v1/automated');
...
function findPython() {
  const candidates = [
    ...
    path.join(AUTOMATED_DIR, 'py_xldiff_compare_months_changes/.venv/bin/python'),
    'python3',
  ].filter(Boolean);
...
export const NO_PYTHON =
  ...
  'automated/py_xldiff_compare_months_changes/.venv, or set XLDIFF_PYTHON to an ' +
