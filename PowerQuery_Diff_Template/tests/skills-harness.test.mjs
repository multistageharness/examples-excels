...
describe('each skill loads into the harness and yields the expected output', () => {
  for (const skill of skills) {
    test(`${skill.dirName}: loaded as instruction document, structured answer confirmed`, {
...
describe('reference skills load into the harness as instruction documents', () => {
  for (const skill of skills.filter((s) => isDocSkill(s.name))) {
    test(`${skill.dirName}: its body and every dependency render into the session`, async () => {
      // A doc skill is never runtime-probed, so the coverage that matters is
      // simply that it — and the skills it depends on — reach the model intact.
      const { harness, systemPrompt } = skillHarness({ skill, allSkills: skills });
      try {
        assert.ok(
          systemPrompt.includes(`<skill name="${skill.name}">`),
          'reference skill was not loaded into the session',
        );
        assert.ok(
          systemPrompt.includes(skill.body.trim()),
          'reference skill body was truncated on the way to the model',
        );
        for (const dep of skill.dependencies) {
          assert.ok(
            systemPrompt.includes(`<skill name="${dep}">`),
            `dependency ${dep} was not loaded alongside ${skill.name}`,
          );
        }
      } finally {
