...
The behavior is specified by `REQ.md` in the reference project
(`v1/automated/py_xldiff_compare_months_changes/` in this repo), and every rule in
`py-xlsx-diff-commons/reference/CONTRACT.md` cites the requirement it implements.
...
One more, in a different key: **[primitives](primitives/SKILL.md)** is a prose-only *reference*
skill — no scripts. It catalogs every building block of the engine (the `reader` / `diff` /
`writer` / `cli` data types, functions, and constants) with the knowledge behind each: what it
guarantees, why it exists, and which primitive owns a given rule. Read it to understand or modify
the engine at the level of its parts, or to port it faithfully. The runnable skills are the *what*;
`primitives` is the *how it's built*.
